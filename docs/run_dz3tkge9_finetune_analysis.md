...
## Fine-tune config — the fix

`configs/trm_official_sudoku_mlp_finetune.yaml` — sister of the
paper-faithful config. Diffs (everything else identical):
...
| `training.q_loss_weight` | 0.5 | **0.01** | per `config.py:131-133` |
| `training.epochs` | 500 | **150** | FT plateaus early |
| `training.eval_interval` | 50 | **10** | catch peak in time for early-stop |
| `training.save_interval` | 20 | **10** | match eval cadence |
| `training.early_stop_patience` | 0 | **10** | don't burn 5 kWh past peak again |
| `training.early_stop_metric` | val_cell_acc | **val_puzzle_acc** | the metric that matters |
...
(no `--epochs` override — let the config's 150 + early-stop govern).

The expected outcome with this config is *at best* a small gain over
0.8484, *at worst* an early-stop-triggered exit at the init level after
~10 evals (~100 epochs ≈ 4–5 h on this hardware) once it's clear nothing
is improving.

## Open questions

1. **Was the released checkpoint trained with `q_loss_weight=0.5` or 0.01?**
   If 0.5, the fine-tune mismatch is smaller than (3) suggests. The
   Sanjin2024 `all_config.yaml` should answer this — worth pulling once.
...
3. **Should we just *not* fine-tune?** The released checkpoint at 0.8484
   ...
   cite the loaded checkpoint. Fine-tuning is only worth attempting if the
   coursework deliverable specifically requires custom training.

## Files written by this analysis

- `configs/trm_official_sudoku_mlp_finetune.yaml` — fix-config
- `scripts/plot_sudoku_mlp_finetune_failure.py` — generates the figure below
- `results/figures/sudoku_mlp_finetune_failure.png` — three-panel diagnostic plot
- `docs/run_dz3tkge9_finetune_analysis.md` — this document
